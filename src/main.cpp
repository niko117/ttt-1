...
#if defined(NDEBUG)
#error "NXBoost cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uNXB) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "NXBoost Signed Message:\n";
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zNXB without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zNXB spend with serial %s is already in block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zNXB spend tx %s already in block %d",
            ...
            // Check that zNXB mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if(nHeight <= 86400 && nHeight > 0) {
		...
		nSubsidy = 50 * COIN;
	} else if (nHeight > 408960 && nHeight <= 411840) { // 408961 => 1 NXBoost FOR THE FIRST FEW HOURS OF POS AND LAST FEW HOURS OF POW
		nSubsidy = 1 * COIN;
...
void RecalculateZNXBSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zNXB supply
...
bool RecalculateNXBSupply(int nHeightStart)
{
            ...
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    ...
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zNXB serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    if (CZXlqStake* zNXB = dynamic_cast<CZXlqStake*>(stake)) {
        ...
        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zNXB stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zNXB->GetValue()));
        if (nChecksum200 != zNXB->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zNXB->GetChecksum(), nChecksum200);
    } else {
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zNXB mints and %d zNXB spends\n", __func__, nMints, nSpends);
