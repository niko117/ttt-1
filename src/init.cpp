...
#include "config/nxboost-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "nxboost.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "nxboostd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the NXB and zNXB money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 49095, 49102));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in NXB/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in NXB/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via HyperSend) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via HyperSend) in <address>"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, nxboost, (obfuscation, hypersend, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in NXB/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all NXBoost specific functionality (Masternodes, Zerocoin, HyperSend, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-NXBstake=<n>", strprintf(_("Enable or disable staking functionality for NXB inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zNXBstake=<n>", strprintf(_("Enable or disable staking functionality for zNXB inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:49095"));
    ...
    strUsage += HelpMessageOpt("-backupzNXB=<n>", strprintf(_("Enable automatic wallet backups triggered after each zNXB minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zNXBbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zNXB backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizenxboostamount=<n>     " + strprintf(_("Keep N NXB anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageGroup(_("HyperSend options:"));
    strUsage += HelpMessageOpt("-enablehypersend=<n>", strprintf(_("Enable HyperSend, show confirmations for locked transactions (bool, default: %s)"), "true"));
    strUsage += HelpMessageOpt("-hypersenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), nHyperSendDepth));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 55000, 51475));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The NXBoost Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that NXBoost is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!GetBoolArg("-enablehypersend", fEnableHyperSend)) {
        if (SoftSetArg("-hypersenddepth", "0"))
            LogPrintf("AppInit2 : parameter interaction: -enablehypersend=false -> setting -nHyperSendDepth=0\n");
    }
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. NXBoost Core is shutting down."));
...
#endif
    // Make sure only a single NXBoost process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. NXBoost Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //NXBoost: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //NXBoost specific: zerocoin and spork DB's
                ...
                // NXBoost: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of NXBoost Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart NXBoost Core to complete") << "\n";
        ...
        zwalletMain = new CzNXBWallet(pwalletMain->strWalletFile);
...
//    }
//
//    nAnonymizeNXBoostAmount = GetArg("-anonymizenxboostamount", 0);
//    if (nAnonymizeNXBoostAmount > 999999) nAnonymizeNXBoostAmount = 999999;
//    if (nAnonymizeNXBoostAmount < 2) nAnonymizeNXBoostAmount = 2;

    fEnableHyperSend = GetBoolArg("-enablehypersend", fEnableHyperSend);
    nHyperSendDepth = GetArg("-hypersenddepth", nHyperSendDepth);
    nHyperSendDepth = std::min(std::max(nHyperSendDepth, 0), 60);
    ...
    LogPrintf("nHyperSendDepth %d\n", nHyperSendDepth);
    LogPrintf("Anonymize NXBoost Amount %d\n", nAnonymizeNXBoostAmount);
    ...
    /* Denominations
       ...
       For example:
       1NXB+1000 == (.1NXB+100)*10
       10NXB+10000 == (1NXB+1000)*10
    */
